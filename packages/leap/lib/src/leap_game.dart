...
class LeapGame extends FlameGame with HasTrackedComponents {
  ...
  /// Backing field for leapMap
  LeapMap? _leapMap;

  /// Safely accessed leapMap getter
  LeapMap get leapMap {
    assert(_leapMap != null, 'leapMap accessed before initialization.');
    return _leapMap!;
  }
  ...
  Future<void> loadWorldAndMap({
    required String tiledMapPath,
    String prefix = 'assets/map/',
    AssetBundle? bundle,
    Images? images,
    Map<String, TiledObjectHandler> tiledObjectHandlers = const {},
  }) async {
    // Remove the existing map if already added
    if (_leapMap != null && _leapMap!.isMounted) {
      _leapMap!.removeFromParent(); // no await needed
      _leapMap = null;
    }

    // Load the new map
    final newMap = await LeapMap.load(
      tileSize: tileSize,
      tiledMapPath: tiledMapPath,
      prefix: prefix,
      bundle: bundle,
    ...
    // Assign and add to world
    _leapMap = newMap;
    await world.add(_leapMap!);
  }
}